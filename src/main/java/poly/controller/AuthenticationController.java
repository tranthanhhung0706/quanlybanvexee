...
//=======
	...
	public String signin(ModelMap model, HttpSession session, HttpServletRequest request,
			@Validated @ModelAttribute("Account") Account account, BindingResult errors) {
...
//		}
	...
	public String register(ModelMap model, @ModelAttribute("tks") Account tk, @ModelAttribute("role") Role role,
			@ModelAttribute("kh") User kh, @RequestParam("name") String name, @RequestParam("gioitinh") String gioitinh,
		...
		if(email.trim().equals("")) {
			check++;
			model.addAttribute("mailerror", "Nhập email");
		}else if(!email.trim().matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
			check++;
			model.addAttribute("mailerror", "Email không hợp lệ");
		}
		if(!cmnd.trim().equals("") && !cmnd.trim().matches("[0-9]{9}")) {
			check++;
			model.addAttribute("cmnderror", "CMND có 9 số");
		}
		if (check != 0)
			return "site/register";

		
		 User usertemp=this.getsdtuser(sdt); 
		 if(usertemp != null) {
			 model.addAttribute("thanhcong","trungsdt");
			return "site/register";
		 }
		...
		tk.setIdRole(role);
		kh.setIdTaiKhoan(tk);
		Integer check3 = this.saverole(tk,role,kh);
...
//		Integer check2 = this.savekh(kh);
		if (check3 == 1) {//check1 == 1 && check2 == 1 && 
			...
			model.addAttribute("thanhcong", "thatbai");
			return "site/register";
		}
	}
	...
	public Integer saverole(Account tk,Role role,User kh) {
		Session session = factory.openSession();
		Transaction t = session.beginTransaction();
		try {
			session.save(role);
